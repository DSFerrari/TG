  ...
  const handleCadastro = async () => {
    ...
    if (existingEmail) {
      Alert.alert(
        ...
        "Esse e-mail já está em uso. Se você não confirmou, verifique sua caixa de entrada."
      );
      return;
    }
    ...
    if (!dataFormatada) {
      Alert.alert(
        "Data inválida",
        "Informe uma data real entre 1900 e o ano atual."
      );
      return;
    }

    const deficienciasString =
      deficienciasMultiplas.length > 0 ? deficienciasMultiplas : [];
